...
import { Package, Wrench, Tag, Users, FileText, X, Plus, Edit, Trash2, Save, ArrowLeft } from 'lucide-react';
...
export default function AdminDashboard({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const [activeTab, setActiveTab] = useState<'categories' | 'products' | 'equipment' | 'orders' | 'users'>('categories');
  ...
  const [newCategory, setNewCategory] = useState({ name: '', type: 'product' as 'product' | 'equipment' });
  ...
  const handleUpdateEquipment = async () => {
    ...
    if (error) {
      ...
      setEquipment(equipment.map(eq => eq.id === editingEquipment.id ? editingEquipment : eq));
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-6">
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-4">
                  <select
                    ...
                    onChange={(e) => setNewCategory({...newCategory, type: e.target.value as 'product' | 'equipment'})}
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {categories.map((category) => (
                      <tr key={category.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{category.id.slice(0, 8)}</td>
                          ...
                          <>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <select
                                ...
                                onChange={(e) => setEditingCategory({...editingCategory, type: e.target.value as 'product' | 'equipment'})}
          ...
          {activeTab === 'products' && (
            <div>
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Добавить новый товар</h2>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {products.map((product) => (
                      <tr key={product.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{product.id.slice(0, 8)}</td>
          ...
          {activeTab === 'equipment' && (
            <div>
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Добавить новое оборудование</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <input
                    type="text"
                  ...
                  <input
                    ...
                    onChange={(e) => setNewEquipment({...newEquipment, daily_rate: parseFloat(e.target.value) || 0})}
                  ...
                  <input
                    ...
                    onChange={(e) => setNewEquipment({...newEquipment, deposit_amount: parseFloat(e.target.value) || 0})}
                    placeholder="Депозит"
                    className="border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  />
                  <input
                  ...
                  <select
                    ...
                    onChange={(e) => setNewEquipment({...newEquipment, is_available: e.target.value === 'available'})}
                    className="border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  >
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Название</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Дневная ставка</th>
                      ...
                      <tr key={item.id}>
                        ...
                        {editingEquipment?.id === item.id ? (
                          <>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <input
                                ...
                                onChange={(e) => setEditingEquipment({...editingEquipment, daily_rate: parseFloat(e.target.value) || 0})}
                                className="border border-gray-300 rounded-lg px-3 py-1 text-sm w-full"
                              />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <input
                                ...
                                onChange={(e) => setEditingEquipment({...editingEquipment, deposit_amount: parseFloat(e.target.value) || 0})}
                                className="border border-gray-300 rounded-lg px-3 py-1 text-sm w-full"
                              />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <select
                                ...
                                onChange={(e) => setEditingEquipment({...editingEquipment, is_available: e.target.value === 'available'})}
                                className="border border-gray-300 rounded-lg px-3 py-1 text-sm"
                              >
                                <option value="available">Доступно</option>
                                <option value="unavailable">Недоступно</option>
                          ...
                          <>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${item.daily_rate.toFixed(2)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${item.deposit_amount.toFixed(2)}</td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                item.is_available 
                                  ? 'bg-green-100 text-green-800' 
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {orders.map((order) => (
                      <tr key={order.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{order.user_id.slice(0, 8)}</td>
